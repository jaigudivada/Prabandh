...
// Prevent production startup without JWT_SECRET
...
app.post('/api/issues', authenticate, upload.array('attachments', 5), (req, res) => {
  try {
    ...
    // Hide reporter identity for anonymous issues from non-admin users
...
app.get('/api/workers', authenticate, authorize('SUPERVISOR', 'ADMIN'), (req, res) => {
  try {
    res.json(workers.filter((w) => w.status === 'ACTIVE'));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/users', authenticate, authorize('ADMIN', 'OFFICIAL'), (req, res) => {
  try {
    res.json(users.map(sanitizeUser).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
    ...
    const data = supervisors.map((s) => {
      ...
      const avgResolutionTime = resolved.length > 0
        ? resolved.reduce((sum, i) => {
          ...
          }, 0) / resolved.length / (1000 * 60 * 60) // hours
        : 0;
      return {
        id: s.id,
        name: s.name,
  ...
  try {
    ...
    const daily = last30Days.map((date) => {
      ...
      return {
        ...
        pending: dayIssues.filter((i) => i.status === 'PENDING').length,
  ...
  try {
    const userStats = users.filter((u) => u.role !== 'ADMIN').map((u) => {
      ...
      return {
        ...
        resolved: resolved.length,
        resolutionRate: reported.length > 0 ? Math.round((resolved.length / reported.length) * 100) : 0,
      };
  ...
  try {
    const deptData = categories.map((cat) => {
      ...
      return {
        ...
        inProgress: catIssues.filter((i) => i.status === 'IN_PROGRESS').length,
        resolved: resolved.length,
        resolutionRate: catIssues.length > 0 ? Math.round((resolved.length / catIssues.length) * 100) : 0,
        avgResolutionTime: Math.round(avgResolutionTime * 10) / 10,
      };
    });
    res.json(deptData);
